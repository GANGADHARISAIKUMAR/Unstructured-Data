...
REQUIRED_MODULES = {
    # Lightweight checks only — skip heavy CV/DeepFace/rembg checks to avoid import-time crashes
    # 'cv2': 'opencv-python-headless, opencv-contrib-python-headless',  # disabled for quick run
}
...
with tab1:
    ...
    if missing:
        st.warning(f"⚠ Missing packages: {', '.join(missing)}. Some features may not work. Check Environment diagnostics above.")
        ...
        if face_btn:
            with st.spinner("👤 Detecting faces..."):
                ...
                try:
                    ...
                    # Fallback to local file or remote download
                    ...
                    min_size = max(30, int(0.08 * min_side))  # at least 30px or 8% of shorter side
                    ...
                    # If no faces with Haar, fallback to DeepFace (retinaface then mtcnn)
                    df_faces = []
                    if len(faces) == 0:
                        try:
                            from deepface import DeepFace
                            # Save to temp path for compatibility
                            with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as tmp:
                                rgb_image.save(tmp.name)
                                temp_path = tmp.name
                            for backend in ['retinaface', 'mtcnn']:
                                try:
                                    extracted = DeepFace.extract_faces(img_path=temp_path, detector_backend=backend, enforce_detection=False)
                                    for f in extracted:
                    ...
                    total_faces = len(faces) + len(df_faces)
                    if total_faces > 0:
                        st.success(f"✅ Detected {total_faces} face(s)!")
                        # Draw rectangles around faces from both detectors
                        face_img = image_np.copy()
                        for (x, y, w, h) in faces:
                            cv2.rectangle(face_img, (x, y), (x+w, y+h), (0, 255, 0), 2)
                        for (x, y, w, h) in df_faces:
                            cv2.rectangle(face_img, (x, y), (x+w, y+h), (255, 165, 0), 2)  # orange for DeepFace
                        st.caption("Green boxes: OpenCV Haar • Orange boxes: DeepFace fallback")
                    ...
                    # Check diagnostics
                    if env_avail.get('cv2') is None:
                        st.info("💡 Fix: Add compatible OpenCV + numpy versions to requirements.txt. Example:\n\nnumpy==1.24.3\nopencv-python-headless==4.9.0.80\nopencv-contrib-python-headless==4.9.0.80")
                    else:
                        st.info("💡 Run: pip install opencv-python-headless opencv-contrib-python-headless")
                except Exception as e:
            ...
            with st.spinner("🎭 Analyzing age, gender & emotion..."):
                ...
                try:
                    ...
                    if "libGL" in str(e):
                        ...
                        st.warning("⚠ No clear face detected in the image. Please use a photo with a visible frontal face.")
                    else:
